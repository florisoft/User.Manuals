...
Build a dynamic form and send the submitted data to one or more email addresses. In addition, configure an automatic confirmation email for the sender. Prevent spam by using reCAPTCHA. Another available option is **Bot Prevention Mode**, which adds a hidden input field to detect and block bots. Optionally, the data of a new customer can be included in an Excel file, allowing it to be easily imported into the back office.

*Follow the steps below:*

|Step|Explanation|
|:--|:--|
|**1**|In this example, we are working on a contact [page](#page) and want to add a form. Click the '+' icon next to the [Panel](#panel) component (1) in the [Navigator](#navigator). This opens the [Component Dialog](#component-dialog).<details><summary><b>Click here for the example image</b></summary><img src="media/52.1.png"></details>|
|**2**|In the dialog, navigate to the **Form** category (2) and click the '+' icon to add the **Form** component to your page (3).<details><summary><b>Click here for the example image</b></summary><img src="media/52.2.png"></details>|
|**3**|The Form component is now visible (4) in the Navigator and can immediately be configured through the **General Settings** tab (5). Use **Edit translations** (6) to enter multilingual text fields. More information can be found in the [Translations](#translations) chapter.<br><br>Enter the **Email Subject** (7) for your form. This is the subject line received by the email addresses entered under **Recipients** (8). Use the add button (9) to specify multiple email addresses.<br><br>There will always be bots attempting to send spam through your forms. You can greatly reduce this by configuring **Bot Prevention Mode** (10) using one of the following options:<br><br>**Hidden input field**: Adds an invisible input field that cannot be seen or filled in by legitimate visitors. If this field is completed (typically by a bot), the form submission is blocked.<br><br>**Time-based**: Bots usually submit forms within milliseconds. If the form is submitted within a configurable time limit (default: 3 seconds), the submission will be rejected.<br><br>If you have your own Google account, you can also enable **Google reCAPTCHA** (11) for your webshop. Follow the steps below:<br><br>**Configuring Google reCAPTCHA**<br><br>1. Log in to your Google Account.<br>2. If your Google Account does not yet have a Cloud Project, create one at: https://console.cloud.google.com/projectcreate<br>3. Create a new reCAPTCHA instance at: https://www.google.com/recaptcha/admin/create<br>3a. Enter a recognizable **Label**, for example: *[Company Name] ReCAPTCHA*.<br>3b. Leave the reCAPTCHA type set to **Score based (v3) - Verify requests with a score**, which is supported by the webshop.<br>3c. Enter your webshop domain under **Domains**, for example: *yourdomain.com*.<br>3d. Select the newly created or an existing Google Cloud Project.<br>3e. Click **Submit**.<br>4. On the next page, save the **Site Key** and **Secret Key**.<br>5. *(Optional)* Click **View in Cloud Console** to access the reCAPTCHA management console.<br>6. Configure the following webshop settings:<br>6a. **RECAPTCHAACTIVE** → Enable.<br>6b. **RECAPTCHASITEKEY** → Enter the Site Key.<br>6c. **RECAPTCHASECRETKEY** → Enter the Secret Key.<br>7. Restart the webshop after saving the webshop settings.<br>8. In CMS Edit Mode, enable **Use reCAPTCHA** in the settings of each form that should use reCAPTCHA.<br>9. Save/publish the CMS changes and refresh the page.<br>10. Google reCAPTCHA is now active. During form submission, Google analyzes the visitor's behavior, browser, and technical details to calculate a score. If the score is too low, the form submission is cancelled, an error message is shown to the user, and no email is sent.<br><br>**Note 1:** This reCAPTCHA implementation does **not** use the traditional checkbox. It performs the verification silently in the background.<br>**Note 2:** Pages containing forms will display a **"Protected by reCAPTCHA"** badge in the bottom-right corner.<br><br>Forms can also be sent using an XSL template to one or more email addresses. In that case, ensure that the webshop's SMTP settings are correctly configured and that an XSL template (12) is available for the active theme. The same applies to the **Autoreply XSL** (13). Read more about [Component Styles](#component-styles) here (14).<details><summary><b>Click here for the example image</b></summary><img src="media/52.3.png"></details>|
|**4**|Confirmation options (15) can easily be configured. Under **Confirmation Feedback** (16), choose whether a **Thank You** message should be displayed using **ShowDialog**, or whether the visitor should be redirected to a [CMS Page](#page). When **CMS Page** is selected, choose the destination page using **CMS Page** (17). If **ShowDialog** is selected, enter a dialog title (18) and message (19) that will be displayed after the form has been successfully submitted.<details><summary><b>Click here for the example image</b></summary><img src="media/52.4.png"></details>|
|**5**|Configure **Mail to sender** (20) if **Send confirmation email** has been enabled in your [Email Input component](#email-input). After submitting the form, the sender will receive a confirmation email. Enter the email title (21) and compose the message (22). You can personalize this message using placeholders such as **{{first_name}}** or **{{company_name}}**. Make sure the placeholder names match the corresponding [input fields](#text-input). We also recommend including a [Checkbox](#checkbox) with the text: *I agree that my personal data may be used to process my request.* to comply with privacy legislation. Finally, specify the email subject (23).<details><summary><b>Click here for the example image</b></summary><img src="media/52.5.png"></details>|
|**6**|Use the [Styling](#styling) tab to configure colors and other visual styling options for the form.<details><summary><b>Click here for the example image</b></summary><img src="media/52.6.png"></details>|
|**7**|The [Advanced](#5-advanced) tab (25) contains additional technical settings for the form. Enable **ExcelAsMailAttachment** (26) to include an Excel file as an email attachment. The **Database** and **ExcelFile** options store the submitted data on the server. The time threshold (27) for the **Time-based** Bot Prevention Mode can also be configured here (in seconds). Optionally assign the form a name (28), which will also appear in the [Form Tracking](#form-tracking) overview. You can also configure a redirect URL (29) after successful submission, although it is recommended to use only one redirect method. Finally, choose whether to clear all input fields after submission (30) or enable **Dummy** mode (31) to use the form in test mode. Additional settings are available under the [Advanced](#5-advanced) tab.<details><summary><b>Click here for the example image</b></summary><img src="media/52.7.png"></details>|
|**8**|Use the [Visibility](#visibility) tab to make the form visible only to specific users.<details><summary><b>Click here for the example image</b></summary><img src="media/52.8.png"></details>|
